...
# HANDLE .VIMRC IF/WHEN NECESSARY (if ~/.vimrc exists and $CHECK_MYTMUX exists; -z is true if it does not exist. that's why the ! at the beginning to reverse make it true if it doesn't not exist). 
...
export CHECK_MYVIMRC=anything
if [ -f ${homefile} ] && [ ! -z ${CHECK_MYVIMRC} ]; then {
