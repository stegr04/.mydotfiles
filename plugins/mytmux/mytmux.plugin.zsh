...
if [ ! -z "${1:-}" ]; then {
	...
	if [ "$1" == "--tmux_unattended" ]; then {
		tmux_unattended=true
	} else {
		tmux_unattended=false
	}
	fi
} else {
...
if [ -f ${HOME}/.tmux.conf ]; then {
  ...
  if ! cmp -s ${SCRIPT_DIR}/.tmux.conf ${HOME}/.tmux.conf; then {
	  warn "Difference detected: ${SCRIPT_DIR}/.tmux.conf ${HOME}/.tmux.conf"
	  #if [ "${tmux_unattended}" == "false" ]; then {
	  	...
	  	if [ "$confirmation" != y ] && [ "$confirmation" != Y ]; then {
            		...
            		vimdiff ${SCRIPT_DIR}/.tmux.conf ${HOME}/.tmux.conf; 
         	}
         	fi
	  
		# So that I don't accidently lose change made to ~/.tmux.conf
		printf '%s ' "Do you want to overwrite ${HOME}/.tmux.conf with ${SCRIPT_DIR}/.tmux.conf? "
		read confirmation
		if [ "$confirmation" = y ] || [ "$confirmation" = Y ]; then {
		  echo "Copying ${HOME}/.tmux.conf to  ${HOME}/.tmux.conf.bak.$(date +%Y-%m-%d_%H:%M:%S | sed 's/\(\.[0-9][0-9][0-9]\)[0-9]*$/\1/' | sed -E 's/((\s)|(:))/./g')"
		  cp ${HOME}/.tmux.conf to  ${HOME}/.tmux.conf.bak.$(date +%Y-%m-%d_%H:%M:%S | sed 's/\(\.[0-9][0-9][0-9]\)[0-9]*$/\1/' | sed -E 's/((\s)|(:))/./g')
	 
		  echo "Overwriting ${HOME}/.tmux.conf with ${SCRIPT_DIR}/.tmux.conf"
		  cp ${SCRIPT_DIR}/.tmux.conf ${HOME}/.tmux.conf
	  	}
	  	fi
	#} elif [ "$tmux_unattended" == "true" ]; then {
		#echo "Copying ${HOME}/.tmux.conf to  ${HOME}/.tmux.conf.bak.$(date +%Y-%m-%d_%H:%M:%S | sed 's/\(\.[0-9][0-9][0-9]\)[0-9]*$/\1/' | sed -E 's/((\s)|(:))/./g')"
		  #cp ${HOME}/.tmux.conf to  ${HOME}/.tmux.conf.bak.$(date +%Y-%m-%d_%H:%M:%S | sed 's/\(\.[0-9][0-9][0-9]\)[0-9]*$/\1/' | sed -E 's/((\s)|(:))/./g')
	...
	info "${SCRIPT_DIR}/.tmux.conf ${HOME}/.tmux.conf files were compared and are the same."
  }
  fi
